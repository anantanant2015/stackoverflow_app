...
const CollectivesCard = ({ collectives }) => {
  ...
  return (
    <Card variant="outlined">
      <CardHeader
        title={<Typography variant="subtitle2" sx={{fontWeight: 600, color: '#3b4045'}}>Collectives</Typography>}
        action={
      ...
      <CardContent>
        {collectives.map((collective, index) => (
          <Box key={index} mb={index < 2 ? 2 : 0}>
            ...
            sx={{ fontWeight: 600, color: '#3b4045' }}>{collective.name}</Typography>
            ...
            <Typography variant="body2" color="text.primary" mt={1} sx={{fontSize: '0.75rem'}}>
