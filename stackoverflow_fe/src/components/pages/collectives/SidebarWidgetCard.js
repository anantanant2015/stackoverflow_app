...
import { Card, CardContent, Typography, Box, Divider, Link, Stack } from '@mui/material';
...
const SidebarWidgetCard = () => {
  const data = {
    ...
    meta: [
      {
        title: 'Changes to reporting for the [status-review] escalation process',
      ...
      {
        title: 'A discussion about closed (and potentially useful) posts on Stack Overflow',
          ...
          <Stack spacing={1} mt={0}>
            {data.blog.map((item, index) => (
              <Box key={index} display="flex" alignItems="flex-start" gap={1}>
                <CreateIcon fontSize="small" sx={{ color: 'black', fontSize: '0.75rem', pt: '10px' }} />
                <Link
                  href={item.href}
                  target="_blank"
                  underline="hover"
                  color="inherit"
                >
                  <Typography variant="body2" color="text.secondary" mt={1} sx={{fontSize: '0.75rem'}}>
                    {item.title}
                  </Typography>
